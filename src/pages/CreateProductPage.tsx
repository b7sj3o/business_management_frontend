...
const CreateProductPage: React.FC = () => {
	...
	useEffect(() => {
		...
		fetchData()
	}, [barcodeFromScanner, showModal, filteredProducers])
	...
	const filterProducers = (type: string) => {
		if (productForeignKeys) {
			const filtered = productForeignKeys.producers.filter(
				producer =>
				...
				<div className='form-group'>
					...
					>
						...
						{productForeignKeys?.volumes.map(volume => (
							<option key={volume.id} value={volume.value}>
								{volume.value}
							</option>
						))}
					</select>
				...
				<div className='form-group'>
					...
					>
						...
						{productForeignKeys?.liquid_models.map(liquid_model => (
							<option key={liquid_model.id} value={liquid_model.value}>
								{liquid_model.value}
							</option>
						))}
					</select>
				...
				<div className='form-group'>
					...
					>
						...
						{productForeignKeys?.puffs_amounts.map(puffs_amount => (
							<option key={puffs_amount.id} value={puffs_amount.value}>
								{puffs_amount.value}
							</option>
						))}
					</select>
				...
				<div className='form-group'>
					...
					>
						...
						{productForeignKeys?.cartridge_models.map(cartridge_model => (
							<option key={cartridge_model.id} value={cartridge_model.value}>
								{cartridge_model.value}
							</option>
						))}
					</select>
				</div>
				<div className='form-group'>
					...
					>
						...
						{productForeignKeys?.resistances.map(resistance => (
							<option key={resistance.id} value={resistance.value}>
								{resistance.value}
							</option>
						))}
					</select>
	...
	return (
		<>
			<div className='switcher'>
				<div
					className='switcher-value active'
			...
			<div className='form-container' id='form-one'>
				...
				<form onSubmit={handleSubmit}>
					...
					<div className='form-group'>
						...
						>
							...
							{filteredProducers.map(producer => (
								<option key={producer.id} value={producer.value}>
									{producer.value}
								</option>
							))}
						</select>
					...
					{formData.product_type === 'Картридж' && <>{renderFormCartridge()}</>}

					{formData.product_type === 'Под' && <>{renderFormPod()}</>}
			...
			<div className='form-container' id='form-many'>
				...
				<br />
				<strong className='form-instructions form-instructions-main'>
					Формат для кожної групи продуктів: <br />
					(Вказувати через кому, замість стрілки)
				</strong>
				<br />
				<strong className='form-instructions'>Готова рідина</strong> &gt;
				виробник &gt; мл &gt; (мг) &gt; смак &gt; закупочна ціна &gt; продажна
				ціна &gt; дроп ціна &gt; к-сть
				<br />
				<strong className='form-instructions'>Самозаміс</strong> &gt; виробник
				&gt; (модель) &gt; мл &gt; (мг) &gt; смак &gt; закупочна ціна &gt;
				продажна ціна &gt; дроп ціна &gt; к-сть
				<br />
				<strong className='form-instructions'>Под</strong> &gt; виробник &gt;
				модель &gt; кольори &gt; закупочна ціна &gt; продажна ціна &gt; дроп
				ціна &gt; к-сть
				<br />
				<strong className='form-instructions'>Картридж</strong> &gt; виробник
				&gt; (модель) &gt; опір &gt; закупочна ціна &gt; продажна ціна &gt; дроп
				ціна &gt; к-сть
				<br />
				<strong className='form-instructions'>Одноразка</strong> &gt; виробник
				&gt; кількість тяг &gt; смак &gt; закупочна ціна &gt; продажна ціна &gt;
				дроп ціна &gt; к-сть
				<br />
				<br />
				<form
					className='multiple-products-form'
