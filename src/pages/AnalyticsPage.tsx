		...
		return (
			...
			>
				<div className='sale-details'>
					...
					<p>
						Дата: {new Date(saleDetails.date).toLocaleDateString('uk-UA')},{' '}
						{new Date(saleDetails.date).toLocaleTimeString('uk-UA', {
							hour: '2-digit',
							minute: '2-digit',
						})}
					</p>
					...
					<div key={date} className='sales-list'>
						...
						{(showAllSales
							...
							<div
								...
								className='sale-item'
								id={`sale_${sale.id}`}
								onClick={() => setSaleDetails(sale)}
							>
								<h4>
									{sale.product_name} - {sale.producer_name} -{' '}
									{sale.product_type} <span className='red'>{sale.amount}</span>
								</h4>
								<button
